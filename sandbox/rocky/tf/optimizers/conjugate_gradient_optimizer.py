...
class PerlmutterHvp(object):
    ...
    def update_opt(self, f, target, inputs, reg_coeff, name="update_opt"):
        with enclosing_scope(self._name, name):
            ...
            constraint_grads = tf.gradients(f, xs=params, name="constraint_gradients")
...
class FiniteDifferenceHvp(object):
    def __init__(self, base_eps=1e-8, symmetric=True, grad_clip=None, num_slices=1, name="FiniteDifferenceHvp"):
...
class ConjugateGradientOptimizer(Serializable):
    ...
    def update_opt(self, loss, target, leq_constraint, inputs, extra_inputs=None, name="update_opt",
