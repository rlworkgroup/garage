...
data = snapshotter.load('/home/mishari/dqn_tuning/beamrider/dqn_beamrider_1')
myenv = data['env']
exploration_policy = data['algo'].exploration_policy
exploration_policy.policy._qf.to('cpu'),
env = wrap_deepmind(make_atari('BeamRiderNoFrameskip-v4'), frame_stack=True)
env = GymEnv(env)
...
ep_rewards = np.asarray([])
for i in range(10):
    path = rollout(myenv, exploration_policy, animated=True, speedup=0.2)
    ep_rewards = np.append(ep_rewards, np.sum(path['rewards']))
