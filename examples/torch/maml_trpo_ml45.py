...
def maml_trpo(ctxt, seed, epochs, rollouts_per_task, meta_batch_size):
    ...
    set_seed(seed)
    env = GarageEnv(
        normalize(ML45.get_train_tasks(), expected_action_scale=10.))
    ...
    test_task_names = ML45.get_test_tasks().all_task_names
    test_tasks = [
        GarageEnv(normalize(ML45.from_task(task), expected_action_scale=10.))
