...
def launch_experiments(gpu):
    ...
    instances = [7, 8, 3, 9, 6, 10, 13 , 12]
    for env_name in ["pick-place-v2", "reach-v2", "push-v2"]:
        for i in range(10):
            if not i % 4:
            ...
            instance_name = f'ml1-rl2-{env_name}-{i}'
            bucket = f'ml1/round2/rl2/v2'
            branch = 'avnish-new-metaworld-results-mt1'
            experiment = f'metaworld_launchers/ml1/rl2_ppo_metaworld_ml1.py --env-name {env_name}'
            ######################################################
            launcher_name = f"launchers/launch-experiment-{algorithm}-{env_name}-{i}.sh"
            ...
            if not gpu:
                ...
                launch_command = (f"gcloud beta compute instances create {instance_name} "
                    f"--metadata-from-file startup-script={launcher_name} --zone {zone} "
            ...
            with open(launcher_name, mode='w') as f:
                f.write(script)
