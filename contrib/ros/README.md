# rllab.contrib.ros Instructions
## Introduction
Most of the current environments for reinforcement learning training(e.g. gym's environments) are not particularly 
designed for robot learning. Consequently, most of their environments are not for continuous action space task. It is also hard for robot learning researchers to use the same environment interface when they want to test their 
algorithms on real robots. 

rllab.contrib.ros's environments take a different approach. It's targeted towards providing the same interfaces for training both simulated and real robots. It's developed by Hejia Zhang.
## Support Platform
