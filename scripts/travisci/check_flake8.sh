...
# Otherwise, obtain the lists by comparing against the release-2018.10 branch
# in the repository.
if [[ "${TRAVIS}" == "true" && "${TRAVIS_PULL_REQUEST}" != "false" ]]; then
  ...
  git remote set-branches --add origin release-2018.10
  git fetch
  files_changed="$(git diff origin/release-2018.10 -M --diff-filter={M,R} \
                     --name-only | grep ".*\.py")"
  files_added="$(git diff origin/release-2018.10 -M --diff-filter=A \
                   --name-only | grep ".*\.py")"
fi
