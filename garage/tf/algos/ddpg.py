...
class DDPG(OffPolicyBatchPolopt):
    """
    ...
    def __init__(self,
        ...
        soft target update.
            ...
            policy_weight_decay(float): L2 weight decay factor for parameters of
        the policy network.
        ...
        if not clip_return:
            if replay_buffer_type == Buffer.HER:
                self.clip_return = 1. / (1. - discount)
            else:
    ...
    def init_opt(self):
        with tf.name_scope(self.name, "DDPG"):
            ...
            self.target_qf_f_prob_online, _, _, _ = self.baseline._build_net(
            ...
            with tf.name_scope("setup_target"):
                ...
                qf_init_ops, qf_update_ops = get_target_ops(
                    self.baseline.get_global_vars(),
                    self.baseline.get_global_vars("target_qf"), self.tau)
            ...
            with tf.name_scope("inputs"):
                ...
                ) if self.replay_buffer_type == Buffer.HER else self.input_dims[
                    "observation"]
            ...
            next_qval = self.baseline.get_qval_sym(
            ...
            qval = self.baseline.get_qval_sym(obs, actions, name="q_value")
