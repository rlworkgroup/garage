...
from garage.experiment.checkpointers.utils import get_now_timestamp
from garage.experiment.checkpointers.utils import get_timestamp
...
class DiskCheckpointer(Checkpointer):
    ...
    def load(self, **kwargs):
        ...
        """
        latest_checkpoint, _ = self._get_latest_checkpoint(
            kwargs.keys(), dry=True)
    ...
    def _load(self, **kwargs):
        ...
        """
        checkpoint, timestamp = self._get_latest_checkpoint(
            kwargs.keys(), dry=False)
    ...
    def _get_saved_names(self):
        ...
        if not path.exists(self.exp_dir):
            ...
            return [
                path.join(self.exp_dir, f) for f in listdir(self.exp_dir)
                if self._is_valid_name(f)
            ]
    ...
    def _get_latest_checkpoint(self, obj_names, dry=False):
        ...
        for timestamp in timestamps:
            ...
            for obj_name in obj_names:
                for file in files:
                    if obj_name in file:
                        cp[obj_name] = pkl.load(open(file,
                                                     'rb')) if not dry else ""
