...
def load_class(class_path, superclass=None, prefix_hints=[]):
    ...
    if module_or_class is None:
        raise ValueError(
            "Cannot find module or class under path %s" % class_path)
    if type(module_or_class) == types.ModuleType:
        ...
        if not classes:
            if superclass:
                raise ValueError(
                    ...
                    % (str(superclass), class_path))
            else:
                raise ValueError(
                    'Could not find any classes defined in module %s' %
                    (class_path))
        elif len(classes) > 1:
            if superclass:
                raise ValueError(
                    'Multiple subclasses of %s are defined in the module %s' %
                    (str(superclass), class_path))
            else:
                raise ValueError(
                    'Multiple classes are defined in the module %s' %
                    (class_path))
        else:
            return classes[0]
        ...
        if superclass is None or issubclass(module_or_class, superclass):
            ...
            raise ValueError('The class %s is not a subclass of %s' %
                             (str(module_or_class), str(superclass)))
    else:
