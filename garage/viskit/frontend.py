...
def summary_name(exp, selector=None):
    ...
    #     exclude_params = set()
    # rest_params = set(
    #     [x[0] for x in distinct_params]).difference(exclude_params)
                ...
                if (only_show_best or only_show_best_final
                    ...
                    if best_regret != -np.inf:
                        ...
                        if use_median:
                            ...
                            if smooth_curve:
                                percentile25 = sliding_mean(
                                    percentile25, window=window_size)
                                percentile50 = sliding_mean(
                                    percentile50, window=window_size)
                                percentile75 = sliding_mean(
                                    percentile75, window=window_size)
...
def index():
    ...
    return flask.render_template(
        ...
        distinct_params=dict(
            [(str(k), list(map(str, v))) for k, v in distinct_params]),
    )
