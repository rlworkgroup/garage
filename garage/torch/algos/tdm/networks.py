...
class TdmNormalizer(object):
    def __init__(
        ...
        self.distance_normalizer = TorchFixedNormalizer(env.goal_dim
                                                        if vectorized else 1)
    ...
    def forward(
        ...
        if self.tdm_normalizer is not None:
            observations, actions, goals, num_steps_left = (
                self.tdm_normalizer.normalize_all(observations, actions, goals,
                                                  num_steps_left))

        predictions = super().forward(observations, actions, goals,
                                      num_steps_left)
    ...
    def forward(
        ...
        if self.tdm_normalizer is not None:
            observations, _, goals, num_steps_left = (
                self.tdm_normalizer.normalize_all(observations, None, goals,
                                                  num_steps_left))
