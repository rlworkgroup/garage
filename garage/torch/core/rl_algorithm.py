...
from garage.torch.samplers.in_place import InPlacePathSampler
...
class RLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def _end_epoch(self):
        logger.log(
            "Epoch Duration: {0}".format(time.time() - self._epoch_start_time))
    ...
    def _handle_rollout_ending(self):
        ...
        if not self._current_path_builder.is_empty():
