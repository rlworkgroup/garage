...
def save_heightfield(x, y, hfield, fname, path=None):
    """
    @param path, str (optional). If not provided, DEFAULT_PATH is used. Make
    sure the path + fname match the <file> attribute of the <asset> element in
    the env XML where the height field is defined
    """
...
def save_texture(x, y, hfield, fname, path=None):
    """
    @param path, str (optional). If not provided, DEFAULT_PATH is used. Make
    sure this matches the <texturedir> of the <compiler> element in the env XML
    """
    path = _checkpath(path)
    plt.figure()
