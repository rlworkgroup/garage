...
class RL2NPO(RL2):
    """RL^2 with inner algorithm NPO (Natural Policy Gradient Optimization).
    ...
    Args:
        ...
        episode_per_task (int): Number of episodes to be sampled per task.
    ...
    def _build_policy_loss(self, i):
        ...
        with tf.name_scope('policy_loss'):
            adv = compute_advantages_meta_learn(
                self.discount,
                self.gae_lambda,
            ...
            returns = discounted_returns_meta_learn(self.discount,
                                                    self.max_path_length,
                                                    self._episode_per_task,
                                                    rewards)
    ...
    def _get_baselines_prediction(self, samples_data):
        ...
        lengths = samples_data['lengths']
        baselines = np.zeros((self._num_of_env, self.max_path_length))
