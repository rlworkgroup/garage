...
class NPO2(BatchPolopt):
    ...
    def _build_inputs(self):
        ...
        with tf.name_scope('inputs'):
            ...
            valid_var = tf.compat.v1.placeholder(tf.float32,
                                                 shape=[None, None],
                                                 name='valid')
            length_var = new_tensor(name='length', ndim=1, dtype=tf.int32)
        ...
        if self.policy.recurrent:
            ...
            if self.policy._state_include_action:
                ...
                for k in self.policy.state_info_keys:
                    ...
                    _obs_var = tf.concat(axis=-1,
                                         values=[_obs_var, extra_state_var])
            _action_var = action_var
        else:
            _obs_var = tf.boolean_mask(obs_var, valid_var)
            _action_var = tf.boolean_mask(action_var, valid_var)

        self.policy.build(_obs_var)
        self._old_policy.build(_obs_var)

        policy_loss_inputs = graph_inputs(
            'PolicyLossInputs',
            action_var=_action_var,
    ...
    def _build_policy_loss(self, i):
        ...
        with tf.name_scope('policy_loss'):
            ...
            if not self.policy.recurrent:
                adv = tf.boolean_mask(adv, i.valid_var)
    ...
    def _build_entropy_term(self, i):
        ...
        if not self.policy.recurrent:
            sparse_policy_entropy = tf.RaggedTensor.from_row_lengths(
                policy_entropy, i.length_var).to_tensor()
            paddings = [[
                0, 0
            ], [0, self.max_path_length - tf.shape(sparse_policy_entropy)[1]]]
            policy_entropy = tf.pad(sparse_policy_entropy, paddings,
                                    'CONSTANT')
