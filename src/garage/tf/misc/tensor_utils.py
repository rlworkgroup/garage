...
def discounted_returns_meta_learn(discount, max_len, episode_per_task,
                                  rewards):
    ...
    rewards_pad = tf.expand_dims(tf.concat(
        [reshaped_rewards,
         tf.zeros_like(reshaped_rewards[:, :-1])], axis=1),
                                 axis=2)
    returns = tf.nn.conv1d(rewards_pad,
                           return_filter,
                           stride=1,
...
def center_advs_local(advs, axes, eps, max_len, name=None):
    ...
    with tf.name_scope(name, 'center_adv_local', [advs, axes, eps, max_len]):
        ...
        advs = (advs - mean) / (tf.sqrt(variance) + eps
...
def compute_advantages_meta_learn(discount,
    ...
    with tf.name_scope(
        ...
        baselines_pad = tf.concat([
            reshaped_baseline[:, 1:],
            tf.zeros_like(reshaped_baseline[:, :1])
        ],
                                  axis=1)
        ...
        deltas_shift = tf.concat(
            [reshaped_deltas,
             tf.zeros_like(reshaped_deltas[:, :-1])], axis=1)
        # Convolve deltas with the discount filter to get advantages
