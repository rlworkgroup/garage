...
class RL2Sampler(BatchSampler):
    ...
    Args:
        ...
        meta_batch_size (int): Number of meta batch for sampling.
        episode_per_task (int): Number of episode for each task.
    ...
    def start_worker(self):
        """Start workers."""
        n_envs = self._n_envs
        assert n_envs >= self._meta_batch_size, (
            'Number of vectorized environments '
            'should be at least meta_batch_size.')
        assert n_envs % self._meta_batch_size == 0, (
            'Number of vectorized '
    ...
    def obtain_samples(self, itr, batch_size=None, whole_paths=True):
        ...
        if batch_size is None:
            batch_size = (self._episode_per_task * self.algo.max_path_length *
                          self._n_envs)
