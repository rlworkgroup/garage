...
try:
    import tensorflow
    from garage.tf.plotter import Plotter as TFPlotter  # noqa: E501; pylint: disable=ungrouped-imports
...
class Trainer:
    ...
    Note:
        ...
        Trainer detects the import module from sys.modules and creates
        session if 'tensoflow' is imported.
    ...
    def __init__(self, snapshot_config, sess=None):
        ...
        # only used for TensorFlow algorithms
        self._tf = 'tensorflow' in sys.modules
        if self._tf:
            self.sess = sess or tensorflow.compat.v1.Session()

            if tensorflow.compat.v1.get_default_session() is not self.sess:
                self.sess.__enter__()

    def __del__(self):
        """Close TensorFlow session if needed."""
        if self._tf and tensorflow.compat.v1.get_default_session(
        ) is self.sess:
            self.sess.close()
    ...
    def initialize_tf_vars(self):
        ...
        with tensorflow.name_scope('initialize_tf_vars'):
            uninited_set = [
            ...
            self.sess.run(
                tensorflow.compat.v1.variables_initializer([
                    v for v in tensorflow.compat.v1.global_variables()
    ...
    def setup(self,
        ...
        For TensorFlow algorighms, it also initializes all uninitialized
    ...
    def _start_worker(self):
        ...
        if self._plot:
            if self._tf:
                self._plotter = TFPlotter(
                    self.get_env_copy(),
                    self._algo.policy,
                    sess=tensorflow.compat.v1.get_default_session())
                self._plotter.start()
...
class __FakeTrainer:
    ...
    def __init__(*args, **kwargs):
        raise ImportError('Please install TensorFlow before using it.')


if not tensorflow:
    Trainer = __FakeTrainer  # noqa: F811
