...
class DiscreteMLPQFunction(MLPModule):
    ...
    def forward(self, observations):
        ...
        For each observation, the network computes the Q value for each
        possible action.

        Args:
            observations (np.ndarray): Batch of observations with
                shape :math:`(N, O)`

        Returns:
            torch.Tensor: tensor with shape :math:`(N, A)` of Q values.
        """
